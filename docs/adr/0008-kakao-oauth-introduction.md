...
**Date**: 2026-05-18
**Status**: accepted (머지: 2026-05-20, PR #61)
**Deciders**: pistachio8
...
- **Supabase auth(GoTrue) 의 카카오 provider 가 `account_email` 을 서버 측 default scope 로 hardcoded** (`internal/api/provider/kakao.go`) 한다. 클라이언트의 `signInWithOAuth({ scopes })` 는 append-only 로 합쳐지므로 `account_email` 을 코드로 배제할 수 없다 — 운영 요구사항: **카카오 콘솔 동의항목에 `카카오계정(이메일)` 을 "선택 동의" 로 반드시 등록**해야 한다(미등록 시 "동의 항목 설정이 잘못됐어요" 에러로 OAuth 차단). Supabase Dashboard 의 "Allow users without an email" 옵션은 사용자가 비동의한 케이스를 흡수해 가입 진행을 보장한다. "필수 동의" 로 격상하거나 다른 비즈 권한이 필요해질 때만 비즈 앱 검수 트랙 진행.
