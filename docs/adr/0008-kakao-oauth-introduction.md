...
- **V1 후속**: events 테이블 partial unique index, profile_image scope, callback 실패 graceful fallback, welcome 배너 sessionStorage 1회 제한, 다른 OAuth provider(Apple 등) 도입 시 본 ADR 패턴 재사용.
