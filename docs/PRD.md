...
- **AC-1** 그룹장은 아래 필드로 챌린지를 생성할 수 있다 (그룹당 동시 1개 — `pending|accepted|active` 합쳐 1개. partial unique index `challenges_one_open_per_group` 가 강제, 위반 시 conflict 토스트):
  ...
  - 예정 벌금 (0~10,000원, 1,000원 단위 — 0원 허용). 근거: D-007 (#58 결정)
    > **벌금 누적 단위 = 주(week)**: `목표 횟수`는 "주 N회"(주간 빈도)이고, 벌금은 **목표를 못 채운 주마다 1회씩 누적**된다(전체 기간 1회 평가 아님). 확정 벌금은 끝난 주만 반영해 단조 증가하고, 진행 중인 주의 위험은 회복 가능. 7일 미만 자투리 주는 목표를 일수 비례로 환산(`ceil(주N회 × 남은일수/7)`). 모델 전문: [spec 2026-06-02-weekly-penalty-accrual](./superpowers/specs/2026-06-02-weekly-penalty-accrual.md).
...
> 활동 종류당 12~18개. **감정 · 강도 · 환경 · 결과**의 4축 믹스로 구성. POC 중 변경 금지.
> 활동 5종 (운동 4 + 식단 1) — v1.0 (운동 4종) freeze · v1.1 release 2026-05-22 (meal 추가, [ADR-0015](adr/0015-meal-activity-type.md)).
> 분석 분기점은 `KEYWORD_POOL_VERSION` 상수가 `keywords_shown` · `action_logged` 이벤트 props 에 담겨 보존.
...
| 🥗 식단 | 샐러드 · 단백질 · 야채듬뿍 · 집밥 · 한그릇 · 정성차림 · 탄단지 · 느린식사 · 물한잔 · 건강한맛 · 도시락 · 함께먹는              |
...
> 위는 7일(1주) 챌린지 예시다. **다주(multi-week) 챌린지는 정산이 끝난 주마다 반복** — 미달 주 수만큼 벌금이 누적되고(확정·단조 증가), 진행 중인 주는 아직 합계에 들어가지 않는다(회복 가능). 현황판은 주차별 칩 + 이번 주 진척 링으로 분리 표시한다. 모델 전문: [spec 2026-06-02-weekly-penalty-accrual](./superpowers/specs/2026-06-02-weekly-penalty-accrual.md).
