...
2. **UX 의도 손실**: "마감 전 1회 사진 교체"(P2-1), "본인 제외 과반 반려"(P2-2), "잔액 부족 시 서약 차단"(P1-1) 같은 **의도된 제약**이 RN 재작성 중 누락되기 쉽다. 화면이 동작하는 것과 *의도대로* 동작하는 것은 다르다.
...
- 단계적 추출: 모노레포 전면 개편을 선행하지 않는다. 기능을 옮길 때 **그 기능이 의존하는 도메인 모듈만** `packages/domain` 으로 점진 이동(00 plan §2 Phase 2).
