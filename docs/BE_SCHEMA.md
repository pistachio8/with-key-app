...
| 10b | `peer_rejections`          | 🟨 익명 피어 반려(토글) — ADR-0038/EVAL-0025      | §5.8.1        |
| 10c | `penalty_proofs`           | 벌칙(만회 찬스) 증명 영상 제출(1인 1제출)         | §5.8.2        |
| 10d | `penalty_proof_rejections` | 🟨 증명 익명 동료 반려(토글) — 0048 미러          | §5.8.2        |
| 10e | `penalty_debts`            | 면제 실패 2X 이월 채무(적재·수금 0056/EVAL-0045)  | §5.8.2        |
| 11  | `push_subscriptions`       | Web Push 구독 토큰                                | §6            |
| 11b | `device_push_tokens`       | RN(Expo) 디바이스 push token — ADR-0041/EVAL-0051 | §5.9.1        |
...
| `reason`       | text        | NO   | —                   | `bundle_grant` · `deposit_hold` · `deposit_release` · `penalty` · `distribution` · `refund` · `penalty_debt_carryover` (0056) |
...
| `media_type`                | text        | NO   | `'photo'`           | `CHECK IN ('photo','video')` (migration 0054). 인증 medium — photo=사진/기존, video=실시간 3초 클립. `challenges.feed_type` 과 정합. **불변**(클라 photo↔video 위조 차단). spec §C2                                   |
| `video_path`                | text        | YES  | null                | (migration 0054) `action-videos` 버킷 경로(`{userId}/{challengeId}/{actionLogId}-{nonce}.{ext}`). `photo_path` 와 동형 — 마감 전 교체 허용(불변 예외 ②). spec §C2                                                     |
...
- **본문 immutability**(migration 0045 + 0046 + 0054): `prevent_action_log_body_mutation` BEFORE UPDATE 트리거가 본문(`challenge_id`·`user_id`·`activity_type`·`selected_keywords`·`shown_keywords`·`reroll_count`·`memo`·`media_type`·`created_at`) 변경을 **클라(anon/authenticated) 에 한해** `42501`로 거부한다(0046 이 서버 경로를 신뢰 예외로 교정 — service_role/SECURITY DEFINER RPC 의 보정·backfill 허용, ADR-0032 "immutability 는 클라가 못 바꾼다"). 0054 가 `media_type` 을 금지 목록에 추가(photo↔video 위조 차단). **예외 2가지만**: ① 검증 status 컬럼군 서버 UPDATE(위 가드가 허용) ② 마감 전 사진/영상 경로 교체(`photo_path`·`video_path` — `update_action_log_photo_path`[0011]·`update_action_log_video_path`[0054] RPC 경로). 사진 교체의 '마감 전·1회' 한정과 부정탐지 재실행은 WP4(EVAL-0024).
...
### 5.8.1 `peer_rejections` 🟨 (ADR-0038 / EVAL-0025 · migration 0048)

🟨 익명 피어 반려. `kudos`와 분리(반려는 표현이 아니라 판정 입력) — emoji 없음.

| 컬럼            | 타입        | Null | Default             | 제약/비고                                                     |
| --------------- | ----------- | ---- | ------------------- | ------------------------------------------------------------- |
| `id`            | uuid        | NO   | `gen_random_uuid()` | PK                                                            |
| `action_log_id` | uuid        | NO   | —                   | FK → `action_logs.id` `on delete cascade`                     |
| `voter_id`      | uuid        | NO   | —                   | FK → `users.id` · **저장하되 어떤 read 도 반환 금지(익명성)** |
| `created_at`    | timestamptz | NO   | `now()`             |                                                               |

- **UNIQUE `(action_log_id, voter_id)`** — 1인 1표·토글 멱등·중복 방지를 스키마 레벨에서 보장.
- **익명성 = "저장 ≠ 노출"**: RLS SELECT 는 본인 행만(`voter_id = auth.uid()`), 카운트는 admin hydrate read 가 `voter_id` 를 select 하지 않고 집계만 노출(ADR-0024 경계).
- **write 정책 없음** → 클라 직접 INSERT/UPDATE/DELETE 전면 deny. write 는 `toggle_peer_rejection` RPC(SECURITY DEFINER) 한 경로만.
- **과반 전이**: 본인 제외 과반(`count > (N-1)/2`, N=서약 참가자) 도달 시 `action_logs.auto_verify_status` `passed → peer_rejected`, 미달 복원 시 `peer_rejected → passed`(단일 쌍). 48h 윈도우(종료 `coalesce(closed_at, end_at)` + 48h) 이후 토글 무효. 그룹장도 일반 1표(전용 권한·`manual_review` 없음).

### 5.8.2 `penalty_proofs` · `penalty_proof_rejections` · `penalty_debts` (ADR-0039 / EVAL-0044 · migration 0055)

벌칙(만회 찬스) 리뎀션 — 미달자가 그룹장 미션을 수행한 증명 영상을 **창2(종료+48h~96h)** 에 제출하고, 동료가 익명 과반으로 "불성실"을 판정한다(`peer_rejections` 미러). 면제 실패분은 2X로 다음 정산에 이월(`penalty_debts` — 적재는 `finalize_penalty_proof`, 수금은 `settle_challenge` carry-over, migration 0056/EVAL-0045).

**`penalty_proofs`** — 증명 제출(challenge당 미달자 1인 1행).

| 컬럼           | 타입        | Null | Default             | 제약/비고                                              |
| -------------- | ----------- | ---- | ------------------- | ------------------------------------------------------ |
| `id`           | uuid        | NO   | `gen_random_uuid()` | PK                                                     |
| `challenge_id` | uuid        | NO   | —                   | FK → `challenges.id` `on delete cascade`               |
| `user_id`      | uuid        | NO   | —                   | FK → `users.id` (수행자)                               |
| `media_path`   | text        | NO   | —                   | action-videos 경로 · CHECK `char_length 10~512`        |
| `status`       | text        | NO   | `'pending'`         | CHECK `in ('pending','accepted','rejected','expired')` |
| `submitted_at` | timestamptz | NO   | `now()`             | 재제출 시 갱신                                         |
| `created_at`   | timestamptz | NO   | `now()`             |                                                        |

- **UNIQUE `(challenge_id, user_id)`** — 그룹장 단일 미션을 미달자 공통 수행 → 1인 1제출(재제출=update).
- **status 라이프사이클**: `pending ↔ rejected`(토글 과반), `accepted`/`expired`는 창2 만료(종료+96h) 확정만(EVAL-0045+ cron).

**`penalty_proof_rejections`** 🟨 — 증명 익명 동료 반려(`peer_rejections` 동형).

| 컬럼         | 타입        | Null | Default             | 제약/비고                                                     |
| ------------ | ----------- | ---- | ------------------- | ------------------------------------------------------------- |
| `id`         | uuid        | NO   | `gen_random_uuid()` | PK                                                            |
| `proof_id`   | uuid        | NO   | —                   | FK → `penalty_proofs.id` `on delete cascade`                  |
| `voter_id`   | uuid        | NO   | —                   | FK → `users.id` · **저장하되 어떤 read 도 반환 금지(익명성)** |
| `created_at` | timestamptz | NO   | `now()`             |                                                               |

- **UNIQUE `(proof_id, voter_id)`** · 인덱스 `(proof_id)`. 익명성·write=RPC 경계는 `peer_rejections`(§5.8.1)와 동일.
- **과반 전이**: 본인(수행자) 제외 과반(`count > (N-1)/2`) 시 `penalty_proofs.status` `pending → rejected`, 미달 복원 `rejected → pending`. 창2 `[종료+48h, +96h]` 밖 토글 무효.

**`penalty_debts`** — 면제 실패(rejected/expired) 2X 이월 채무. 적재 = `finalize_penalty_proof`(창2 만료), 수금 = `settle_challenge` carry-over(같은 그룹 다음 정산, migration 0056).

| 컬럼                  | 타입        | Null | Default             | 제약/비고                                                     |
| --------------------- | ----------- | ---- | ------------------- | ------------------------------------------------------------- |
| `id`                  | uuid        | NO   | `gen_random_uuid()` | PK                                                            |
| `user_id`             | uuid        | NO   | —                   | FK → `users.id`                                               |
| `origin_challenge_id` | uuid        | NO   | —                   | FK → `challenges.id` `on delete cascade` (그룹 스코핑 도출원) |
| `amount`              | integer     | NO   | —                   | CHECK `> 0` (2X)                                              |
| `status`              | text        | NO   | `'open'`            | CHECK `in ('open','settled')`                                 |
| `created_at`          | timestamptz | NO   | `now()`             |                                                               |
| `settled_at`          | timestamptz | YES  | null                | 수금 시각(EVAL-0045)                                          |

- 부분 인덱스 `(user_id) where status='open'` — 수금 후보 조회. UNIQUE `(user_id, origin_challenge_id)` — finalize 1회 적재 멱등(0056). client write 경로 없음(`finalize_penalty_proof` service_role 적재 · `settle_challenge` 수금, RLS 우회 definer).
- **RPC**(SECURITY DEFINER · `search_path=public`): `submit_penalty_proof(challenge_id, media_path)` — 창2·서약·벌칙 챌린지 검증 후 upsert. `toggle_penalty_proof_rejection(proof_id)` — `toggle_peer_rejection`(0048) 동일 과반식 미러.
...
### 5.9.1 `device_push_tokens` (RN Expo)

> RN(React Native) 전환용 — [ADR-0041](./adr/0041-rn-push-token-model.md) · migration 0058 · EVAL-0051. 기존 `push_subscriptions`(Web Push)는 cutover(Phase 8)까지 공존하고, dispatch sender가 두 테이블을 모두 조회해 web=`web-push`, expo=Expo Push Service(`exp.host`)로 발송한다(수신자 선정·quiet hours·dedup 로직은 불변).

| 컬럼              | 타입        | Null | Default             | 제약/비고                                  |
| ----------------- | ----------- | ---- | ------------------- | ------------------------------------------ |
| `id`              | uuid        | NO   | `gen_random_uuid()` | PK                                         |
| `user_id`         | uuid        | NO   | —                   | FK → `users.id` ON DELETE CASCADE          |
| `device_id`       | text        | NO   | —                   | 기기 식별자(expo-device installation id)   |
| `expo_push_token` | text        | NO   | —                   | UNIQUE — `ExponentPushToken[...]`          |
| `platform`        | text        | NO   | —                   | CHECK `in ('ios','android')`               |
| `app_version`     | text        | YES  | null                |                                            |
| `last_seen_at`    | timestamptz | YES  | null                |                                            |
| `disabled_at`     | timestamptz | YES  | null                | 무효 토큰 soft-delete(DeviceNotRegistered) |
| `created_at`      | timestamptz | NO   | `now()`             |                                            |

- UNIQUE `(user_id, device_id)` — 기기당 1행(토큰 갱신은 upsert). UNIQUE `(expo_push_token)` — 기기 이전/재설치 정리.
- 무효 토큰은 hard-delete 대신 `disabled_at` soft-delete — 재등록 시 `(user_id, device_id)` upsert로 재활성. dispatch는 `disabled_at IS NULL`만 발송 대상으로 본다.
...
| `photo_path`  | text        | YES  | null                | (deprecated, 0049) `feedback-photos` 버킷 단일 경로. `photo_paths[0]` 미러 — 슬랙 단일 미리보기 하위호환 |
| `photo_paths` | text[]      | NO   | `'{}'`              | (0049) 사진 경로 배열, CHECK `array_length ≤ 3`. 멀티 사진 SoT. 빈 배열 = 사진 없음                      |
| `created_at`  | timestamptz | NO   | `now()`             |                                                                                                          |
...
- **사진**: 신규 private 버킷 `feedback-photos`(5MB, jpeg/png/webp). owner-scoped storage RLS(INSERT/SELECT/DELETE `auth.uid() = foldername[1]`). 0011 `action-photos` 와 동형이되 경로가 2-segment. **0049 부터 최대 3장**(`photo_paths[]`) — 같은 `feedbackId` 로 nonce 만 달리해 기존 경로 규격·storage RLS 그대로 지원(정책 변경 없음). 첨부 전부를 Slack #qa 에 노출(ADR-0035 amendment).
...
| `idx_peer_rejections_action_log` ON `peer_rejections(action_log_id)`                                           | 🟨 피드 카드당 반려 카운트 집계 (0048)                              |
...
| `idx_dpt_user` ON `device_push_tokens(user_id)`                                                                | 발송 대상 조회(Expo · ADR-0041)                                     |
...
| `uq_point_ledger_carryover_ref` (UNIQUE) ON `point_ledger(ref_id) WHERE reason = 'penalty_debt_carryover'`     | debt 1건당 carry-over 차감 1행 — 이중수금 backstop (0056)           |
...
| `peer_rejections` 🟨          | **본인 행만** (`voter_id=auth.uid()`, 익명성) | ❌ 정책 없음(`toggle_peer_rejection` RPC only)                                   | ❌ 정책 없음(RPC only)                                                        | ❌ 정책 없음(RPC only)             |
| `penalty_proofs`              | 그룹 멤버                                     | ❌ 정책 없음(`submit_penalty_proof` RPC only)                                    | ❌ 정책 없음(RPC only)                                                        | ❌ 정책 없음                       |
| `penalty_proof_rejections` 🟨 | **본인 행만** (`voter_id=auth.uid()`, 익명성) | ❌ 정책 없음(`toggle_penalty_proof_rejection` RPC only)                          | ❌ 정책 없음(RPC only)                                                        | ❌ 정책 없음(RPC only)             |
| `penalty_debts`               | 본인 또는 그룹 멤버                           | ❌ 정책 없음(service_role only · EVAL-0045)                                      | ❌ 정책 없음                                                                  | ❌ 정책 없음                       |
| `push_subscriptions`          | self                                          | self                                                                             | self                                                                          | self                               |
| `device_push_tokens`          | self                                          | self                                                                             | self                                                                          | self                               |
...
| `settle_challenge(challenge)`                      | 그룹장(owner)·cron(auto) | `settlements` insert `on conflict do nothing`(0행이면 no-op) → 참가자별 release(+held)+penalty(-forfeit), `pool_points=Σforfeit`, 분배 스냅샷. 벌칙 챌린지는 penalty deferred. **같은 그룹 open `penalty_debts` 를 `penalty_debt_carryover`(-amount)로 1회 수금**(pool 합산·debt settled, 0056) | `settlements.challenge_id` PK · carry-over `ref_id=debt.id` |
| `finalize_penalty_proof(challenge)`                | service_role(cron) 전용  | 창2 만료(종료+96h) 후 — pending→accepted flip, 미달자별 accepted→면제 / rejected·미제출(expired)→`penalty_debts(2X, open)` 적재 (0056)                                                                                                                                                          | `(user_id, origin_challenge_id)`                            |
...
| `0054_action_videos.sql`                          | 영상 인증(spec §C2 / EVAL-0043) — 신규 private 버킷 `action-videos`(MIME mp4/webm; `.mov` 는 0059 에서 추가) + storage RLS(av\_\*, action-photos 미러) + `action_logs.media_type`·`video_path` 컬럼 + 0046 불변 트리거 갱신(`media_type` 추가) + `update_action_log_video_path` RPC. spec 예약번호 0052→0054 재부여(0052·0053 선점)        |
| `0055_penalty_redemption.sql`                     | 벌칙 redemption 스키마(spec §C3·C4 / EVAL-0044) — `penalty_proofs`·`penalty_proof_rejections`·`penalty_debts` 테이블+RLS + `submit_penalty_proof`·`toggle_penalty_proof_rejection` RPC(peer-reject 미러). 적재·수금은 0056 으로 deferred                                                                                                   |
| `0056_redemption_settlement_carryover.sql`        | 벌칙 redemption 정산 연동(spec §C5 / EVAL-0045) — `point_ledger.reason` CHECK +`penalty_debt_carryover` · `penalty_debts` UNIQUE(user,origin) · carry-over 부분 UNIQUE 인덱스 · `finalize_penalty_proof` RPC(service_role 적재) · `settle_challenge` forward `create or replace`(같은 그룹 open debt 2X carry-over 수금, INSERT-once 보존) |
| `0057_challenge_montages.sql`                     | 합본 몽타주 fast-follow(spec §C6-B / EVAL-0046 · ADR-0040) — 신규 private 버킷 `challenge-montages`(MIME mp4, 경로 `{challengeId}/montage.mp4`) + storage RLS `cm_select_group_member`(그룹 멤버 select만; write 정책 부재 = 워커 service_role 전용). Oracle A1 ffmpeg 워커가 push, recap 이 viewer user client 로 read                    |
| `0058_device_push_tokens.sql`                     | RN(Expo) push token 모델(ADR-0041 / EVAL-0051) — `device_push_tokens` 신설(`(user_id × device_id)` 매핑, `expo_push_token` UNIQUE, 무효 토큰 soft-delete `disabled_at`) + RLS `dpt_all_self`(self 만, `push_subscriptions.ps_all_self` 미러). `push_subscriptions`(Web Push)는 cutover(Phase 8)까지 공존                                   |
| `0059_action_videos_allow_mov.sql`                | action-videos 영상 포맷 확장(spec §C2 / EVAL-0062) — 버킷 `allowed_mime_types` 에 `video/quicktime`(.mov) 추가(RN iOS expo 카메라) + `update_action_log_video_path` RPC 파일명 정규식 `(mp4\|webm)`→`(mp4\|webm\|mov)`. RLS·`submit_penalty_proof`(0055, 확장자 미검사) 무영향                                                             |
...
- **v0.8** (2026-06-18) — **건의 사진 멀티(최대 3)** (Ian · [ADR-0035](./adr/0035-feedback-table-storage.md) amendment · migration 0049)
  - §5.11: `feedback.photo_paths text[]`(NOT NULL `'{}'`, CHECK `array_length ≤ 3`) 추가 — 멀티 사진 SoT. `photo_path`(단일)는 deprecated(`photo_paths[0]` 미러, 슬랙 단일 미리보기 하위호환).
  - storage 정책 변경 없음 — 같은 `feedbackId`/다른 nonce 로 기존 owner-scoped RLS·2-segment 경로 규격이 멀티를 이미 지원. 첨부 전부를 Slack #qa 에 노출.
