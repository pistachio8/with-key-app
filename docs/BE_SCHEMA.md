...
| 10e | `penalty_debts`            | 면제 실패 2X 이월 채무(적재·수금 EVAL-0045)     | §5.8.2        |
...
벌칙(만회 찬스) 리뎀션 — 미달자가 그룹장 미션을 수행한 증명 영상을 **창2(종료+48h~96h)** 에 제출하고, 동료가 익명 과반으로 "불성실"을 판정한다(`peer_rejections` 미러). 면제 실패분은 2X로 다음 정산에 이월(`penalty_debts` — 적재·수금은 EVAL-0045).
...
**`penalty_debts`** — 면제 실패(rejected/expired) 2X 이월 채무(테이블만; 적재·수금 EVAL-0045).
...
- 부분 인덱스 `(user_id) where status='open'` — 수금 후보 조회(EVAL-0045). client write 경로 없음(service_role cron 적재 예정).
