...
| `0054_action_videos.sql`                          | 영상 인증(spec §C2 / EVAL-0043) — 신규 private 버킷 `action-videos`(MIME mp4/webm) + storage RLS(av\_\*, action-photos 미러) + `action_logs.media_type`·`video_path` 컬럼 + 0046 불변 트리거 갱신(`media_type` 추가) + `update_action_log_video_path` RPC. spec 예약번호 0052→0054 재부여(0052·0053 선점)                                  |
