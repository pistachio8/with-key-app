...
| Q8 (신규)  | full CI 강제 라벨 escape hatch | **`run-full-ci` 라벨로 docs-only · draft 양쪽 모두 무시하고 풀 파이프라인 실행** | 라벨 기본은 사용 안 함(누락 위험 해소). 의심스러운 PR 또는 release prep 시 명시적 opt-in. \*\*라벨의 의미와 `if:` 조건은 반드시 일치 — 즉 `(draft == false && has_code == 'true')       |     | label`처럼 라벨이 OR 의 동급 분기로 위치해야 함**.`draft == false && (has_code |     | label)` 형태는 라벨이 draft 를 뚫지 못해 의미와 조건이 어긋난다 → 금지 형태로 명시 |
