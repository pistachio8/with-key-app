...
describe("MemberRoster", () => {
  it("멤버 전원의 displayName 표시 (달성·미달성 차이 없음)", () => {
    render(
      <MemberRoster
        members={[
          { displayName: "김주은", isMvp: false },
          { displayName: "최소원", isMvp: false },
          { displayName: "이성훈", isMvp: false },
          { displayName: "박지민", isMvp: false },
        ]}
      />,
    );
  ...
  it("isMvp true 멤버 옆에 왕관 아이콘 (aria-label='MVP')", () => {
    render(<MemberRoster members={[{ displayName: "JJ", isMvp: true }]} />);
...
type Props = {
  members: ReadonlyArray<{ displayName: string; isMvp: boolean }>;
};
  ...
  return (
    ...
    >
      ...
      <ul className="grid grid-cols-2 gap-x-4">
        {members.map((m) => (
          <li key={m.displayName} className="font-semibold">
            {m.displayName}
            {m.isMvp && (
              <Crown
                aria-label="MVP"
...
- [ ] **Step 1: Pretendard ttf 자산 위치 확인 또는 등록**

```bash
ls public/fonts/ 2>/dev/null
ls node_modules/pretendard 2>/dev/null
```

Expected: 없으면 `public/fonts/Pretendard-SemiBold.ttf`를 추가하거나, system serif/sans 폴백으로 진행. 누락 시 한글 깨짐 — 본 task 완료 전 반드시 확인.
...
async function loadFont(): Promise<ArrayBuffer | null> {
  ...
  try {
    const buf = await readFile(path.join(process.cwd(), "public/fonts/Pretendard-SemiBold.ttf"));
  ...
  return new ImageResponse(
    (
      ...
      >
        ...
        <div
          style={{ display: "flex", flexDirection: "column", alignItems: "center", fontSize: 22, color: "#5E4838" }}
        >
          ...
          <span style={{ color: "#B07A4D", marginTop: 8 }}>
    ...
    ),
    {
      ...
      height: 1080,
      fonts: font
        ? [{ name: "Pretendard", data: font, weight: 600, style: "normal" as const }]
        : undefined,
    ...
    <div className="flex flex-col gap-4 p-4">
      ...
      {!isSolo && recap.startAt && recap.endAt && (
        <>
          ...
          <MemberRoster
            members={recap.members.map((m) => ({ displayName: m.displayName, isMvp: m.isMvp }))}
          />
          <SettlementAccount
