...
export function DaySlider({ totalDays, currentDay, verifiedDays, onArrive }: DaySliderProps) {
...
- [ ] **Step 5: typecheck (호출부가 깨지는지 확인)**

Run: `pnpm typecheck`
Expected: FAIL — `action-result-dialog.tsx` 가 아직 `verifiedDays` 를 전달하지 않아 타입 에러. Task 8 에서 해소(정상 진행).
...
Run: `pnpm typecheck`
Expected: FAIL — `action-form.tsx` 가 아직 신규 props/variant 를 전달하지 않음(Task 9 에서 해소). dialog 파일 자체 타입 에러는 없어야 함.
...
- **주의(실행 순서):** Task6·8 의 typecheck 는 Task9 완료 전까지 의도적으로 FAIL(상호 의존). 6→7→8→9 를 한 묶음으로 끝내면 Task9 Step7 에서 green. subagent-driven 실행 시 이 묶음을 함께 검토 권장.
