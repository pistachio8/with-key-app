...
- 신규: `docs/adr/0034-feedback-table-storage.md` (migration 동반 ADR)
...
- **왜 업로드 선행(③→④)**: RLS 가 INSERT-only 라 insert-후-update 경로가 없다. orphan row 대신 orphan object 를 택하고 ④ 실패 시 정리한다 — 이 트레이드오프는 ADR-0034 에 기록.
...
1. migration `0047_feedback.sql` + ADR-0034 + 본 spec 을 같은 PR 로 머지 (CI Integration 이 공유 Supabase 에 db push)
...
- `truncate_test_data` 의 기존 잠복 결함(point_ledger·settlements 미정리)은 본 기능과 무관한 별도 forward-fix — ADR-0034 에 인지만 기록
