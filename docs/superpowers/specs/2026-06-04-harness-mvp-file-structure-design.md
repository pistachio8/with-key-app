...
**자기유지 흐름**(Prompt 2 dev-progress): Development Progress → **Task Summary**(`pnpm harness:summarize-diff`) → Harness Impact Check(AT의 6 질문) → Drift Detection(`pnpm harness:drift`) → Drift Report(`evals/drift-reports/`) → Update Proposal(propose-harness-update) → Human Review(review-harness-update) → Update Task(apply-harness-update) → Changelog(`.agents/harness/CHANGELOG.md`). 작업 전 컨텍스트 수집은 `pnpm harness:context`.
...
.agents/
  ...
  migration/
    INDEX.md             →포인터       # 03(매핑 SoT) · 00 · 05
    REVIEW_CHECKLIST.md                # 포팅 PR 리뷰 체크리스트(신규 체크리스트 — 03 복제 아님)
  backlog/
...
scripts/harness-{check★,drift,summarize-diff,context}.mjs   # pnpm(=verify·summarize-diff·collect-context.sh 대체)
...
| `.agents/migration/REVIEW_CHECKLIST.md` | 포팅 PR 리뷰 체크리스트(03 적용 체크, 복제 아님) | review-agent-task(port) | 전환 규칙 변경 시 (Level 2) |
...
| `scripts/harness-summarize-diff.mjs` | diff→Task Summary(Harness Impact 입력) | implement-agent-task, check-harness-drift | 요약 형식 변경 시 (Level 2) |
| `scripts/harness-context.mjs` | AT Source Files+규칙 컨텍스트 번들(=collect-context) | create-agent-tasks, implement-agent-task | 컨텍스트 규칙 변경 시 (Level 2) |

### 4.3.1 핵심 비-★ 파일 seed 골격

비-★ 파일의 *완성* 본문은 구현 단계 산출(§8). 아래는 의도를 고정하는 최소 seed.

- **`.agents/README.md`** — 하네스 1줄 정의 + 진입 맵: "머시너리=`.agents/`, 인스턴스=docs/evals, 작업종류→`workflows/` 매핑표, 검증=`pnpm harness:*`". Codex·Claude 공통 진입점.
- **`.agents/engineering/INDEX.md`** — 포인터 표만: ARCHITECTURE→04·docs/ARCHITECTURE / CODING·TESTING·ERROR·API·STATE→`.claude/rules/*` / DoD→QUALITY_GATE §공통 성공 기준. "본문 복제 금지(ADR-0031)" 1줄.
- **`.agents/migration/INDEX.md`** — "매핑 SoT=`03-rn-migration-rules`" + port/greenfield 비혼합 규율(원칙 9) 1줄 + 00·05 링크.
- **`.agents/migration/REVIEW_CHECKLIST.md`** — 포팅 PR 리뷰 체크박스: `[ ] Track=port 태그 / [ ] 보존 eval pass^k=100% / [ ] feature가 expo-* 직접 import 없음 / [ ] RSC·cache·hydration 잔재 없음 / [ ] 03 매핑 준수 / [ ] Parent 인용 resolve`. (03 규칙의 *적용 체크*지 복제 아님)
- **`.agents/harness/DRIFT_CHECKLIST.md`** — 7 drift 유형 × {시점(Define/Record/sweep)·Tier·결정론 체크 명령} 표. §4.5 check-harness-drift Process가 읽음.
- **`.agents/harness/DECISION_NEEDED.md`** — PO 대기 결정 append-only 로그. 초기 항목: G1 θ(false-flag 임계)·G2 법무·04 §9(invite re-tap UX / Bottom Tabs IA). 각 `{id·차단 task·해소 조건·상태}`.
...
// git diff 를 Task Summary 로 요약 (Development Progress → Task Summary 단계).
// 출력 = 변경 파일·요약 → Harness Impact Check(AT 6 질문) 입력.
```

```js
// scripts/harness-context.mjs  →  pnpm harness:context <task-id>
// Agent Task 의 Source Files + 규칙 포인터를 모아 컨텍스트 번들 생성 (=collect-context.sh 대체).
// create-agent-tasks / implement-agent-task 의 Read First 입력.
```
...
```bash
pnpm harness:context <task-id>   # AT 컨텍스트 번들 수집 (구현 전 — collect-context)
pnpm harness:summarize-diff      # 변경 diff → Task Summary (구현 후 — Harness Impact 입력)
...
6. `harness/`(UPDATE_POLICY·DRIFT_CHECKLIST·DECISION_NEEDED·config) + `harness:{check,drift,summarize-diff,context}` + `evals/{drift-reports,meta}` + migration `REVIEW_CHECKLIST.md`.
7. TRACEABILITY 수동 시드(P0 9개) + 첫 마일스톤 sweep.

## 8. Out of scope

- `scripts/harness-{check,drift,summarize-diff,context}.mjs`·`harness-report`의 **실제 구현**(본 스펙은 계약·골격만 — writing-plans 이후 코드 단계).
- **비-★ 파일의 완성 본문**(README·INDEX·템플릿·harness 문서)은 §4.3.1 seed 골격만 스펙에 두고 완성 본문은 구현 단계 산출. 왜: 스펙 비대화 방지 + 구현이 곧 생성하는 내용 중복 회피.
