...
jest `--testPathPattern`은 `src/` 앵커로 쓴다(예: `src/.*read-only`). **왜**: worktree 디렉토리명(`with-key-rn-read-only-screens`)에 task slug가 들어가 비앵커 패턴이 경로 전체에 오염된다(EVAL-0017 실측). `.agents/templates`의 task 템플릿 주석에 추가.
...
1. **PR-A `feat/harness-orchestration-cli`**: C1~C4 + C7(템플릿 주석) + 테스트. `scripts/**`만.
