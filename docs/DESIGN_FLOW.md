...
- **v1.2** (2026-05-11) — 솔로 챌린지 정식 모드 dual-mode reframing (D-022). 1인 그룹에서도 챌린지 생성·서명·진행 가능. UI 분기: `/pledge` 동의 라벨 · 활성화 토스트(솔로/그룹 + 의도-결과 불일치 Edge #2), `/challenge/[id]` next-step CTA 매트릭스 (pending/accepted × mySigned × solo/group), Kudos footer 미렌더 (participantCount===1), `recap` 솔로 미달성 verdict 재카피, 입구·메타 카피 일반화 ("혼자, 또는 친구와 함께하는 운동 기록"). 데이터 모델 변경 없음. 풀 다이어그램 갱신은 v1.3 에서 일괄.
