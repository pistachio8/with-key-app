...
- 직교 결정(인용만 — 본문 복제 아님):
  ...
  - 게이트: [PRD §7 Q1](../migration/01-rn-mvp-prd.md) (G1 false-flag 임계 θ — 잠정확정·주입 2026-06-05, [spec](../superpowers/specs/2026-06-05-false-flag-threshold-theta.md); 실측 PoC open, DECISION_NEEDED `G1-θ` partial)
...
**게이트 경계**: 데이터 컬럼·가드 트리거·결정론 검사 *골격*은 G1과 무관하게 즉시 구현·테스트(05 §3). false-flag 임계 θ는 **잠정확정·주입**(2026-06-05, [spec](../superpowers/specs/2026-06-05-false-flag-threshold-theta.md))되어 **판정 task는 활성(todo)** — 실측 G1 PoC는 후속 튜닝(코드 변경 없이 주입값 교체, `false_flag_rate.active=false` 유지). 피어 반려·운영 알림은 θ 무관이나, 반려 저장 모델·신규 이벤트는 각각 spec 선행이 필요하다.
...
- **WP2 — 결정론 부정탐지 판정 RPC**: phash 중복(그룹/전역)·EXIF 촬영시각·스크린샷 휴리스틱 신호 계산 → status 결정(기본 `passed`, 명백 부정 `failed`, 경계 `manual_review`; phash scope 차등·shadow mode·server-only env 주입은 [θ spec](../superpowers/specs/2026-06-05-false-flag-threshold-theta.md)). 제출 즉시 판정, 신호·`model_version` 기록(본문 미로깅). `AC-auto-verify-1~4`·`AC-cheat-detect-1,2`. _gate: **G1-θ 해제**(잠정 주입 2026-06-05) — 판정 task 활성(EVAL-0022 todo), 검사 골격·불변식은 EVAL-0021 선행. 실측 PoC 후속._
...
> 의존 순서: WP1 → (WP2 ∥ WP4) · WP3 독립 · WP5 → WP6. WP1·WP3·결정론 골격은 게이트 무관, WP2는 G1-θ 잠정 주입으로 판정 task 활성(실측 PoC 후속), WP5는 reaction spec 선행, WP6은 이벤트 spec 선행.
