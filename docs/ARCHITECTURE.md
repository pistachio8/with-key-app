...
- 도메인 타입은 `packages/domain/src/validators/`(`@withkey/domain`) zod 스키마에서 `z.infer<>`로 도출
...
## 키워드 풀 (`packages/domain/src/keywords/`)
...
## 정산 · recap · 공유 (`packages/domain/src/{challenge,share}` 순수 로직 · apps/web `api/og`·`api/share`)
