...
3. **브랜치** — 현재 브랜치 확인. `develop`/`main` 위에서 사소하지 않은 변경(feat/fix/refactor)을 시작하려면 **먼저 `feat/<scope>-<desc>` · `fix/<scope>-<desc>` · `chore/<desc>` 브랜치를 끊는다**. PR 베이스는 `develop`. `develop` 직접 커밋은 트리비얼 docs/chore(오타·룰 인덱스·의존 버전 bump)에 한함. 정책 전문은 [`docs/ONBOARDING.md §8`](./docs/ONBOARDING.md).
