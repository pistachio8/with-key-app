...
2. [implement-agent-task.md](implement-agent-task.md) 전체를 따른다: Source→Target→**Verify green→항상 fan-out 리뷰(§5 — 무인 경로는 diff 크기와 무관, Phase 4 §C1)→merge+verify→review 증거 기록**→Harness Impact→finalize→worktree 커밋. 구현은 메인 세션 inline(EVAL-0019 방식). tick 종료 시 Output Format 의 **tick 계약**으로 보고한다(Phase 4 §C3).
...
1 tick 종료 시 **tick 계약**(고정 포맷 — 사람이 D6 게이트에서 한눈에 검수, Phase 4 §C3)으로 보고한다:

- **Claimed**: EVAL-XXXX (todo→in_progress 확인)
- **Implemented**: 변경 파일 (Target 만, Non-goals 봉인)
- **Verified**: pass@N + 실행한 Verification Commands·green 결과
- **Reviewed**: fan-out 된 reviewer 목록 + CRITICAL/HIGH 건수 + merge·소스검증 노트
- **Repaired**: N회 (fix-verification 분기 있었으면, 없으면 0)
- **Finalized**: done flip + runs[] 채움(`<<FILL>>` 0) + `harness:check` exit 0
- **Stopped at**: 정지한 D6 게이트(보통 push/PR) 또는 clarify/blocked / `unblockCandidates` flip 요청

Verified·Reviewed 줄은 `harness:finalize` 의 review 증거 backstop(Phase 4 §C2)이 뒷받침한다 — 증거가 runs[] 에 없으면 finalize 가 거부되므로 계약은 신뢰가 아니라 강제에 근거한다.
