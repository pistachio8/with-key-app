...
7. **커밋 권한 경계** — implementer 는 WP worktree 안에서 커밋까지 자율로 수행한다. 푸시·PR 생성은 오케스트레이터가 사용자 확인 후에만. **왜**: worktree 내 커밋은 로컬·가역적 — 저장소 밖으로 나가는 행위(push·PR)만 사람 게이트로 충분하다 (`AGENTS.md` §8 worktree 예외와 동기, spec orchestration-phase2 §C5).
8. **CI 모니터** — 푸시 후 `gh pr checks <PR번호> --watch` 로 성공/실패 결론까지 감시한다. 실패 시 로그 확인 → 수정 → 재푸시 → 재감시. **왜**: 로컬 green ≠ CI green 실측 (PR #216 — 로컬 `harness:verify` 2회 통과 후 CI jest 타임아웃).
