...
// PRD §9.1 — 진입 시 action_started 분석 이벤트(markActionStarted) 발화. server 측
// idempotency 가 "1일 1회"를 보장한다(분석 dedupe). 그룹원 인증 완료 푸시는 제출 시
// submitActionLog 에서 발송한다. 발화는 MarkActionStartedOnMount(클라이언트 mount 시
// 1회) 로 위임 — 과거 page 본문에서 fire-and-forget 으로 호출하던 시절 RSC prefetch · HMR
// · RSC payload 재요청마다 withUser→auth.getUser 가 다중 호출되며 GoTrue
// `over_request_rate_limit` (429) 의 메인 기여자가 되어 분리.
