...
// --json: 오케스트레이터용 구조화 출력 { ready, unblockCandidates, inProgress }.
// 기본: 사람용 요약 — waiting 까지 보여준다(JSON 은 spec 형태 고정, 사람 뷰는 진단용으로 더 넓게).
// unblockCandidates 는 보고만 한다 — blocked→todo flip 은 사람 몫 (D6, 자동 flip 아님).
import { loadMigrationTasks, resolveReadyTasks } from "./harness-lib.mjs";

const tasks = loadMigrationTasks();
const { ready, waiting, inProgress, unblockCandidates } = resolveReadyTasks(tasks);

if (process.argv.includes("--json")) {
  console.log(JSON.stringify({ ready, unblockCandidates, inProgress }, null, 2));
  process.exit(0);
}
...
- READY: ${ready.length} · WAITING: ${waiting.length} · in_progress: ${inProgress.length} · unblock candidates: ${unblockCandidates.length}
`);
...
if (ready.length + waiting.length + inProgress.length + unblockCandidates.length === 0) {
