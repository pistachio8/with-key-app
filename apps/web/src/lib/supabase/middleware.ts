...
export async function updateSession(request: NextRequest) {
  ...
  if (!url) throw new Error("NEXT_PUBLIC_SUPABASE_URL is required for session middleware");
  if (!key)
    throw new Error("NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY is required for session middleware");
