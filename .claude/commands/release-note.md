...
1. **범위 확정** — `gh pr list --state merged --base main --json number,title,mergedAt` 로 대상 PR 번호·제목·머지일 수집. 머지일이 여러 날이면 가장 늦은 날을 파일명으로. **왜 `main`**: release-note 는 실제 production(`main`)에 반영된 변경만 공지한다. develop 머지(dogfood 단계)는 아직 사용자에게 배포 전일 수 있어 제외.
