...
RE_PATH_REF = re.compile(
    r"(?<![A-Za-z0-9_/.\-])"
    r"((?:\./|\.?[A-Za-z0-9_]+/)[A-Za-z0-9_./-]+\.(?:tsx|jsx|yaml|html|toml|json|py|ts|js|md|sql|yml|css|sh|go|rs|java|kt|rb|php))"
)
...
def find_core_modules(repo: Path, exclude: set[str] | None = None) -> list[Module]:
    ...
    for d in sorted(repo.iterdir()):
        ...
        if d.name in IGNORE_DIRS or d.name.startswith("."):
            continue
        if d.name in exclude or str(d.relative_to(repo)) in exclude:
            continue
        candidates.append(d)
        ...
        if parent.exists() and parent.is_dir():
            ...
            for d in sorted(parent.iterdir()):
                if d.is_dir() and d.name not in IGNORE_DIRS:
                    ...
                    if d.name in exclude or rel in exclude:
                        continue
                    candidates.append(d)
...
def score_c(modules: list[Module], repo: Path) -> CategoryScore:
    ...
    has_adr = any(p.exists() for p in adr_candidates)
...
def score_e(repo: Path, context_files: list[Path]) -> CategoryScore:
    ...
    for p in context_files:
        text = read_text(p)
        # strip URLs so their tail segments don't masquerade as repo paths
        text_no_urls = re.sub(r"https?://\S+", " ", text)
        for ref in set(RE_PATH_REF.findall(text_no_urls)):
            total_refs += 1
...
# ----------------------------------------------------------------------------
def find_large_files(repo: Path, threshold: int = 300, exclude: set[str] | None = None) -> list[tuple[Path, int]]:
    exclude = exclude or set()
    ...
    for p in walk_files(repo):
        ...
        if top in exclude or any(rel_str.startswith(e.rstrip("/") + "/") for e in exclude):
            continue
        ln = count_lines(p)
        if ln > threshold:
            out.append((p, ln))
...
def build_report(repo: Path, exclude: set[str] | None = None) -> Report:
    exclude = exclude or set()
    modules = find_core_modules(repo, exclude=exclude)
    context_files = find_all_context_files(repo)
    root_claude = find_root_claude(repo)
    large_files = find_large_files(repo, 300, exclude=exclude)
...
def main() -> int:
    ...
    p.add_argument("--exclude", default="", help="comma-separated top-level dirs to exclude from module scan and god-file scan (e.g. 'public,dist')")
    args = p.parse_args()
    ...
    report = build_report(repo, exclude=exclude)
