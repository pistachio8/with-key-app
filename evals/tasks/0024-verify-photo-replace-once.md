...
> Work Package WP4 (`feat/rn-verify-replace`). 외부 게이트 없음 → `todo`(선례 EVAL-0006: intra-feature 의존은 blocked 아님). **WP1/EVAL-0020 의존**(immutability 예외 컬럼·트리거). 교체 시 부정탐지 재실행은 EVAL-0021 신호 골격 위에서 동작(활성 판정은 G1/EVAL-0022 후속).

## Parent Links

- Parent PRD Feature: `AC-auto-verify-5`(마감 전 1회 한정 사진 교체 허용, 교체 시 부정탐지 재실행 — immutability 예외, Q9) — [docs/migration/01-rn-mvp-prd.md](../../docs/migration/01-rn-mvp-prd.md) §5.B
...
단순 실수로 잘못 올린 인증을 억울하게 날리지 않게 한다. 이 task가 끝나면 사용자가 **마감 전 딱 1회** 자신의 인증 사진을 교체할 수 있고, 교체 시 EVAL-0020의 immutability 좁은 예외를 통해 사진 경로가 갱신되며 부정탐지 신호(EVAL-0021)가 재실행되고, 2회째 교체와 마감 후 교체는 차단된다.
...
- 본인 인증 사진 **마감 전 1회만** 교체 — 2회째·마감 후 차단(서버 가드).
- 교체는 Server Action(`_actions.ts`)으로 일원화(클라 직접 write 금지 — 아키텍처 가드레일).
- 교체 시 부정탐지 신호 재실행(EVAL-0021) — phash/EXIF/스크린샷 재계산.
- immutability 예외 경로는 EVAL-0020 트리거 정의 범위 내(사진 교체만, 본문 불변).

## Non-goals

- immutability 예외 _migration·트리거 정의_ — WP1/EVAL-0020 (본 task는 그 예외를 _사용_).
- 교체 후 status 활성 판정 — WP2b/EVAL-0022 (G1 후속; 본 task는 신호 재실행까지).
- 피어 반려와의 상호작용 — WP5/EVAL-0025.

## Acceptance Criteria

| 기준                                  | 검증 방법                                                       |
| ------------------------------------- | --------------------------------------------------------------- |
| 마감 전 1회 교체 (`AC-auto-verify-5`) | 1회 교체 성공 → 사진 경로 갱신 (Server Action 테스트)           |
| 2회째 차단                            | 동일 인증 2회 교체 시도 → 거부                                  |
| 마감 후 차단                          | 마감 시각 이후 교체 시도 → 거부                                 |
| 부정탐지 재실행                       | 교체 시 EVAL-0021 신호 재계산 호출 확인                         |
| Server Action 경로                    | 교체 write가 `_actions.ts` 경유(클라 직접 write 부재) 코드 대조 |
...
사진 교체 Server Action 위치, 1회 제한·마감 전 가드 구현, 부정탐지 재실행 hook, immutability 예외 사용 범위, EVAL-0020 의존을 한국어로 요약한다.
...
- 1회 제한·마감 가드·재실행 Acceptance Criteria green + 모바일 viewport 확인 + `pnpm harness:check` 통과.
- pass@3 안에 green 못 만들면 → 교체 가드 / 재실행으로 split (프롬프트·컨텍스트 1회 점검 후, 05 §9.4).
